...
def check_dependencies() -> ValidationCheck:
    ...
    try:
        ...
        if all_installed:
            return ValidationCheck("Dependencies", True, f"All {len(installed)} required packages installed")
        else:
        ...
        return ValidationCheck(
            "Dependencies", False, f"Could not check dependencies: {e}", "Run: pip install -r requirements.txt"
        )
...
def check_config_directory() -> ValidationCheck:
    ...
    if not config_dir.exists():
        try:
            ...
            return ValidationCheck("Config Directory", True, f"Created config directory: {config_dir.absolute()}")
        except Exception as e:
            return ValidationCheck(
                "Config Directory",
                False,
    ...
    try:
        ...
        test_file.unlink()
        return ValidationCheck(
            "Config Directory", True, f"Config directory exists and is writable: {config_dir.absolute()}"
        )
    except Exception as e:
        return ValidationCheck(
            "Config Directory",
...
def check_default_config() -> ValidationCheck:
    ...
    if config_file.exists():
        return ValidationCheck("Default Config", True, f"Found default configuration: {config_file.absolute()}")
    else:
        return ValidationCheck(
            "Default Config",
            False,
...
def check_camera_access() -> ValidationCheck:
    ...
    try:
        ...
        if cap.isOpened():
            ...
            if ret and frame is not None:
                ...
                if len(camera_indices) >= 2:
                    return ValidationCheck(
                        "Camera Access", True, f"Found {len(camera_indices)} cameras at indices: {camera_indices}"
                    )
                else:
                    return ValidationCheck(
                        "Camera Access",
        ...
        return ValidationCheck(
            "Camera Access", False, f"Error accessing cameras: {e}", "Check camera drivers and permissions"
        )
...
def check_recordings_directory() -> ValidationCheck:
    ...
    if not recordings_dir.exists():
        try:
            ...
            return ValidationCheck(
                "Recordings Directory", True, f"Created recordings directory: {recordings_dir.absolute()}"
            )
        except Exception as e:
            return ValidationCheck(
    ...
    try:
        ...
        test_file.unlink()
        return ValidationCheck(
            "Recordings Directory", True, f"Recordings directory exists and is writable: {recordings_dir.absolute()}"
        )
    except Exception as e:
        return ValidationCheck(
            "Recordings Directory",
...
def check_disk_space() -> ValidationCheck:
    ...
    try:
        ...
        if free_gb >= 10:
            return ValidationCheck("Disk Space", True, f"{free_gb:.1f} GB available (sufficient for recordings)")
        elif free_gb >= 1:
            return ValidationCheck(
                "Disk Space", True, f"{free_gb:.1f} GB available (limited - consider freeing space for recordings)"
            )
        else:
            return ValidationCheck(
                "Disk Space",
        ...
        return ValidationCheck("Disk Space", True, "Could not check (psutil not installed - not critical)")
    except Exception as e:
        return ValidationCheck("Disk Space", True, f"Could not check: {e} (not critical)")
