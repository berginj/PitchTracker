    ...
    try:
        ...
        for i, dev in enumerate(devices):
            ...
            if is_arducam:
                arducam_count += 1
                logger.info(
                    f"[Attempt {attempt}]   ⭐ UVC {i}: {friendly} [Mfg: {manufacturer}] (SN: {serial}) - ARDUCAM"
                )
            else:
    ...
    for idx in range(max_index):
        ...
        try:
            ...
            if cap.isOpened():
                ...
                logger.info(
                    f"[Attempt {attempt}]   Camera {idx}: OPENED in {elapsed:.2f}s (backend={backend}, can_read={can_read})"
                )
                results.append(
                    {"index": idx, "opened": True, "time": elapsed, "backend": backend, "can_read": can_read}
                )
            else:
                cap.release()
                logger.debug(f"[Attempt {attempt}]   Camera {idx}: Failed to open in {elapsed:.2f}s")
                results.append({"index": idx, "opened": False, "time": elapsed})

        except Exception as e:
            elapsed = time.time() - start
            logger.debug(f"[Attempt {attempt}]   Camera {idx}: Exception after {elapsed:.2f}s: {e}")
            results.append({"index": idx, "opened": False, "time": elapsed, "error": str(e)})
...
def main():
    ...
    print(
        f"  Min: {min(all_results['uvc'])}, Max: {max(all_results['uvc'])}, Avg: {sum(all_results['uvc'])/len(all_results['uvc']):.1f}"
    )
    ...
    print(
        f"  Min: {min(all_results['opencv_parallel'])}, Max: {max(all_results['opencv_parallel'])}, Avg: {sum(all_results['opencv_parallel'])/len(all_results['opencv_parallel']):.1f}"
    )
    ...
    print(
        f"  Min: {min(all_results['opencv_sequential'])}, Max: {max(all_results['opencv_sequential'])}, Avg: {sum(all_results['opencv_sequential'])/len(all_results['opencv_sequential']):.1f}"
    )
    ...
    print(
        f"  Min: {min(all_results['direct_open'])}, Max: {max(all_results['direct_open'])}, Avg: {sum(all_results['direct_open'])/len(all_results['direct_open']):.1f}"
    )
    ...
    print(
        f"  Min: {min(all_results['arducam_mapping'])}, Max: {max(all_results['arducam_mapping'])}, Avg: {sum(all_results['arducam_mapping'])/len(all_results['arducam_mapping']):.1f}"
    )
