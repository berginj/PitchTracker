...
def create_icon():
    ...
    margin = 20
    draw.rounded_rectangle([margin, margin, size - margin, size - margin], radius=30, fill=bg_color)
    ...
    ball_y = center_y - 40
    draw.ellipse(
        [ball_x - ball_radius, ball_y - ball_radius, ball_x + ball_radius, ball_y + ball_radius], fill=accent_color
    )

    # Stitching on baseball
    stitch_color = (200, 200, 200)
    draw.arc(
        [ball_x - ball_radius + 5, ball_y - ball_radius + 5, ball_x + ball_radius - 5, ball_y + ball_radius - 5],
        start=-45,
        end=135,
        fill=stitch_color,
        width=3,
    ...
    for point in path_points:
        dot_size = 5
        draw.ellipse(
            [point[0] - dot_size, point[1] - dot_size, point[0] + dot_size, point[1] + dot_size], fill=accent_color
        )
