...
def detect_mode_a(
    ...
    # EMA update — stays float32 throughout, no redundant dtype conversions
    updated_bg = config.bg_alpha * gray + (1 - config.bg_alpha) * background

    components = connected_components(foreground)
    return _components_to_detections(components), updated_bg
