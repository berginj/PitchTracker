...
def main():
    ...
    parser = argparse.ArgumentParser(description="Analyze pitch patterns from recorded sessions")

    subparsers = parser.add_subparsers(dest="command", help="Available commands")

    # analyze-session command
    analyze_parser = subparsers.add_parser("analyze-session", help="Analyze a single session")
    analyze_parser.add_argument(
        "--session", required=True, help="Path to session directory (e.g., recordings/session-2026-01-19_001)"
    )
    analyze_parser.add_argument("--pitcher", help="Pitcher ID/name (optional)")
